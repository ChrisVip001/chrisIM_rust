...
/// 自动生成的User服务gRPC客户端
#[derive(Clone)]
pub struct UserServiceGrpcClientGen {
    service_client: GrpcServiceClient,
}

impl UserServiceGrpcClientGen {
    /// 创建新的User服务客户端
    ...
    /// 获取底层服务客户端
    async fn get_client(&self) -> Result<UserServiceClient> {
        let channel = self.service_client.get_channel().await?;
        Ok(crate::proto::user::user_service_client::UserServiceClient::new(channel))
    }
