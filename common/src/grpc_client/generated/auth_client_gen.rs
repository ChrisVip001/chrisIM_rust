...
/// 自动生成的Auth服务gRPC客户端
#[derive(Clone)]
pub struct AuthServiceGrpcClientGen {
    service_client: GrpcServiceClient,
}

impl AuthServiceGrpcClientGen {
    /// 创建新的Auth服务客户端
    ...
    pub fn from_env() -> Self {
        let service_client = GrpcServiceClient::from_env("auth-service");
        Self::new(service_client)
    }

    /// 获取底层服务客户端
    async fn get_client(&self) -> Result<AuthServiceClient> {
        let channel = self.service_client.get_channel().await?;
        Ok(crate::proto::auth::auth_service_client::AuthServiceClient::new(channel))
    }
