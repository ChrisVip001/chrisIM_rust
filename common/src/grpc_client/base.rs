...
impl GrpcServiceClient {
    ...
    pub async fn refresh_channels(&self) -> Result<()> {
        // 从Consul获取服务实例
        let service_urls = self.service_registry.discover_service(&self.service_name).await?;
        
        if service_urls.is_empty() {
            return Err(anyhow::anyhow!("没有发现可用的 {} 服务实例", self.service_name));
        }
        ...
        if new_channels.is_empty() {
            return Err(anyhow::anyhow!("没有可用的 {} 服务实例连接", self.service_name));
        }
        ...
        *channels = new_channels;
        ...
        info!("已更新 {} 服务的 {} 个gRPC连接", self.service_name, channels.len());
        Ok(())
    }
    ...
    /// 启动一个后台任务定期刷新服务实例列表
    pub async fn start_refresh_task(client: Arc<Self>) {
