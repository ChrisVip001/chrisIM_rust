...
pub trait GrpcClientFactory: Send + Sync {
    /// 转发gRPC请求
    fn forward_request(&self, req: Request<Body>, target_url: String) -> BoxFuture<'static, Response<Body>>;
...
impl GrpcClientFactoryImpl {
    ...
    fn parse_path(&self, path: &str) -> (String, String, String) {
        ...
        let service_name = if parts.len() > 2 { parts[2].to_string() } else { "unknown".to_string() };
        let method_name = if parts.len() > 3 { parts[3].to_string() } else { "unknown".to_string() };
    ...
    async fn handle_user_request(&self, method: Method, path: &str, body: Value) -> Result<Response<Body>, anyhow::Error> {
        ...
        match (method, method_name.as_str()) {
            ...
            (Method::PUT, "updateUser") | (Method::PATCH, "updateUser") => {
                ...
                let nickname = body.get("nickname").and_then(|v| v.as_str()).map(|s| s.to_string());
    ...
    fn forward_request(&self, req: Request<Body>, target_url: String) -> BoxFuture<'static, Response<Body>> {
        ...
        Box::pin(async move {
            ...
            match service_name.as_str() {
                "users" => {
                    match self_clone.handle_user_request(method, &path, body).await {
