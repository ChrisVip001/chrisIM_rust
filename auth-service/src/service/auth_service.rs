...
use common::proto::auth::{
    auth_service_server::AuthService, CreateTokenRequest, CreateTokenResponse,
    InvalidateTokenRequest, InvalidateTokenResponse, RefreshTokenRequest, RefreshTokenResponse,
    UserClaims, ValidateTokenRequest, ValidateTokenResponse,
};
use common::{config::AppConfig, utils, Result};
...
impl AuthService for AuthServiceImpl {
    ...
    async fn create_token(
        ...
        let (access_token, refresh_token, expires_in) =
            match self.generate_token_pair(&req.user_id, &req.username).await {
                Ok(tokens) => tokens,
                Err(err) => {
                    error!("生成令牌对失败: {}", err);
                    return Err(err.into());
    ...
    async fn refresh_token(
            ...
            .await
        {
            ...
            Ok(None) => {
                debug!("刷新令牌无效或已过期");
                return Err(common::Error::TonicStatus(Status::unauthenticated(
                    "刷新令牌无效或已过期",
                ))
                .into());
            }
        ...
        // 生成新的令牌对
        let (access_token, refresh_token, expires_in) =
            match utils::validate_jwt(&req.refresh_token) {
                Ok(claims) => match self.generate_token_pair(&user_id, &claims.username).await {
                    Ok(tokens) => tokens,
                    Err(err) => {
                        error!("生成新令牌对失败: {}", err);
                        return Err(err.into());
